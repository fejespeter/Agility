...
public class IrrigationServerTest {
	...
	public void isMoistureNeededTest() {
        ...
        @Test
	public void controllTheIrrigator() throws InterruptedException {
            	Sensor testSensor = new Sensor(0, server);
		server.addSensor(testSensor);
                server.start();
                assertFalse(server.irrigator.commandWasReceivedFromServer);
                Thread.sleep(50*1000);
                assertFalse(server.irrigator.commandWasReceivedFromServer);
                Thread.sleep(51*1000);
                assertTrue(server.irrigator.commandWasReceivedFromServer);
        }
